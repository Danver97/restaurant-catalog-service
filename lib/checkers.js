...
function checkTables(tables, type = {}) {
    ...
    if (type.add && !(tables[0] instanceof Table))
        throw new RestaurantError('Invalid tablesAdded() <tables> method param: should be array of Table or array of string.');
    else if (!type.add && !(tables[0] instanceof Table) && typeof tables[0] !== 'string')
        throw new RestaurantError('Invalid tablesAdded() <tables> method param: should be array of Table or array of string.');
    return false;
}
